...
1. **Нет widget функций** - используются только классы виджетов (не функции, возвращающие Widget)
2. **Локализация** - все тексты хранятся в локализации (`AppLocalizations`)
3. **Стили из темы** - все стили (цвета, шрифты) берутся из `Theme.of(context)`
4. **Разделение ответственности** - каждый класс имеет одну ответственность
5. **Интерфейсы для репозиториев** - использование абстракций вместо конкретных реализаций
6. **Типобезопасность** - использование Freezed для моделей данных
