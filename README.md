...
1. **Domain** - бизнес-логика, сущности, интерфейсы репозиториев, провайдеры репозиториев, use cases
2. **Data** - реализация репозиториев, модели данных, источники данных (Retrofit), провайдеры data source
3. **Notifier** - управление состоянием через Riverpod
4. **UI** - экраны и виджеты

### Провайдеры

- **Провайдеры репозиториев** находятся в `domain/providers/` - возвращают интерфейсы из domain слоя
- **Провайдеры data source** находятся в `data/providers/` - зависят от реализации (Retrofit)
- **Провайдеры конфигурации** находятся в `core/config/` - управление окружениями и моками
